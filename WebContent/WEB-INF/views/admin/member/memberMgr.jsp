...
// 		 $('select[name=search_gugun] option').remove();
// 		   $('select[name=search_gugun]').append('<option value="" selected="selected">선택하세요.</option>');
// 		   var gugunArr = getGugun(sido);
// 		   for(var i = 0; i < gugunArr.length; i++){
// 		      $('select[name=search_gugun]').append('<option value="' + gugunArr[i] + '">' + gugunArr[i] + '</option>');
// 		   }
// 	});
					...
					<form name="search_form" action="/admin/member/memberMgr.do" method="post">
						<table border="0" cellspacing="1" cellpadding="1">
							...
							<tr>
								<th>거주지</th>
								<td>	
									<select style="width: 130px;" name="search_sido" onchange="gugunSet(this.value)">
											<option value="" selected="selected">선택하세요</option>
											<c:forEach items="${sidoList}" var="sido">
											<option value="${sido}" var="sido">${sido}</option>
											</c:forEach>
									</select> 
