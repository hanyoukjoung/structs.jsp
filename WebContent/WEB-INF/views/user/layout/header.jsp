				...
				<ul class="nav navbar navbar-top-links navbar-right mbn">
					...
					<li class="dropdown">
						...
						<ul class="dropdown-menu" role="menu">
							<li><a href="${pageContext.request.contextPath }/member/memberView.do?mem_id=${LOGIN_MEMBERINFO.mem_id}"><i class="fa fa-user"></i>프로필관리</a></li>
							...
							<li class="divider"></li>
							<li><a href="${pageContext.request.contextPath }/join/logout.do"><i class="fa fa-key"></i>로그아웃</a></li>
						</ul>
					</li>
				</ul>
			</div>
	...
	<c:if test="${empty LOGIN_MEMBERINFO}">
			<div class="loginForm nav navbar navbar-top-links navbar-right"
				...
				<ul>
					...
					<li>&nbsp;</li>	
			        <li class="dropdown">
			        	<button type="submit" id="btn" class="btn btn-warning btn-sm">로그인</button>
			        	<button type="button" class="btn btn-error btn-sm" id="btnReg">회원가입</button>
				    </li>
				</ul>	    
			</div>
				</c:if>
...
$(function(){
	$('#btn').click(function(){
		...
		var encryptID = rsaOBJ.encrypt(mem_id);
		var encryptPWD = rsaOBJ.encrypt(mem_pass);

		// 동적인 form 태그 작성 및 서브밋 처리
		var $frm = $('<form name="loginForm" action="${pageContext.request.contextPath }/join/loginCheck.do" method="POST"/></form>');
	...
	$('#btnReg').click(function(){
		location.href = '/join/memberRegist.do';
	});
