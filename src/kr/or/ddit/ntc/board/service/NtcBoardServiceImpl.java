...
public class NtcBoardServiceImpl implements NtcBoardService {
	...
	public NBoardVO nboardInfo(Map<String, String> params) {
		...
		try {
			nboardInfo= dao.nboardInfo(params);
		} catch (SQLException e) {
			e.printStackTrace();
		}
		return nboardInfo;
	...
	@Override
	public void updatenboard(NBoardVO freeboardInfo, List<File> files, 
			List<String> fileNames, List<String> fileContentType) {
		try {
			dao.updatenboard(freeboardInfo);
			
			List<BoardFileVO> fileItemList = AttachFileMapper.mapper(files, fileNames, fileContentType, freeboardInfo.getBo_no());
	...
	@Override
	public void insertnboard(NBoardVO freeboardInfo,List<File> files, 
			List<String> fileNames, List<String> fileContentType) {
		try {
			String bo_no=dao.insertnboard(freeboardInfo);
			
			List<BoardFileVO> fileItemList = AttachFileMapper.mapper(files, fileNames, fileContentType, bo_no);
			filedao.insertNboardFile(fileItemList);
		} catch (SQLException e) {
			e.printStackTrace();
		}
