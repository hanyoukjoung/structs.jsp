...
public class REFBoardServiceImpl implements REFBoardService {
	...
	@Override
	public void insertBoardInfo(RBoardVO boardInfo,
								List<File> files, 
								List<String> fileNames, 
								List<String> fileContentType) {
		try {
			String bo_no = dao.insertBoardInfo(boardInfo);
			List<BoardFileVO> FileList = AttachFileMapper.mapper(files, fileNames, fileContentType, bo_no);
	...
	public void insertRBoardInfoReply(RBoardVO rboardInfo) {
		try {
			dao.insertRBoardInfoReply(rboardInfo);
		} catch (SQLException e) {
			e.printStackTrace();
		}
	}
