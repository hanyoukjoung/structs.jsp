...
public class IIMGBoardServiceImpl implements IIMGBoardService{
	...
	@Override
	public void insertBoard(IBoardVO boardInfo, List<File> files, 
		...
		try {
			...
			fileService.insertBoardFile(fileItemList);
		} catch (SQLException e) {
			e.printStackTrace();
		}
	}
	...
	@Override
	public int updateBoard(IBoardVO boardInfo, List<File> files, 
			List<String> fileNames, List<String> fileContentType) {
		try {
			dao.updateBoard(boardInfo);
			
			List<BoardFileVO> fileItemList = AttachFileMapper.mapper(files, fileNames, fileContentType, boardInfo.getBo_no());
			fileService.insertBoardFile(fileItemList);
