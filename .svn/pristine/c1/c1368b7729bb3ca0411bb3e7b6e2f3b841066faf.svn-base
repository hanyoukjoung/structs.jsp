...
public class REFBoardDaoImpl implements REFBoardDao {
	...
	@Override
	public void insertRBoardInfoReply(RBoardVO rboardInfo) throws SQLException {
		try {
			client.startTransaction();
			
			String bo_seq;
			
			if("0".intern() == rboardInfo.getBo_seq().intern()){
				bo_seq = (String) client.queryForObject("rboard.incrementSEQ", rboardInfo);
			}else{
				client.update("rboard.updateSEQ", rboardInfo);
				bo_seq = String.valueOf(Integer.parseInt(rboardInfo.getBo_seq())+1);
			}
			rboardInfo.setBo_seq(bo_seq);
			
			String bo_depth = String.valueOf(Integer.parseInt(rboardInfo.getBo_depth())+1);
			rboardInfo.setBo_depth(bo_depth);
	
			client.insert("rboard.insertRBoardReply",rboardInfo);
